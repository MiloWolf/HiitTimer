...
let settings = {
  count: 250, // how many times we'll repeat our pattern
  colors: {
    // we'll choose a random color from this list
    1: "rgb(29,0,225)",
    2: "rgb(23,194,226)",
    3: "rgb(50,88,169)",
  },
  bgColor: "rgb(0,0,0)", // background color
  pathSrcIndex: 1, // this determines which path we pull from the pathSrcs array
...
class App {
  ...
  getImagePath() {
    ...
    let color =
      settings.colors[
        1 + Math.floor(Math.random() * Object.keys(settings.colors).length)
      ];
    // add our color to our string
    let path = addStyleToSVGPath(src, `fill:` + color);
    // get our transforms
    let scale =
      Math.random() * (settings.scaleVariance * settings.scaleMin) +
      settings.scaleMin;
    ...
    // and add them
    path = addStyleToSVGPath(
      path,
      `transform: translate3d(${translateX},${translateY},0) rotate(${rotate}) scale(${scale})`
    );
    return path;
  }
}
